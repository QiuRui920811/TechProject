...
| 中文名 | 英文 ID | 說明 | 輸入 → 輸出 | 耗能 |
|---|---|---|---|---|
| 聚變反應爐 | `fusion_reactor` | 聚變環流；兼具發電與合成的混合型機器 | fusion_mesh, stability_core → fusion_core | 28 EU/t |
...
`apex_forge`（巔峰鍛成儀）耗能 **52 EU/t**，是全模組最高階的鍛造機台，用宇宙矩陣、封存核心、連續體外殼與遺產資料壓縮出 `omega_matrix`、`apex_core`、`omega_archive` 等巔峰核心。
