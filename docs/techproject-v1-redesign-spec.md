...
#### 反作弊：非互動時間右鍵 = 烹調失敗
- 舊行為：玩家可全程按住右鍵不放，自動通過所有 QTE 互動，達成「完美烹調」
- 新行為：烹調進行中，非互動窗口 / 非食材追加窗口期間對烹飪台右鍵 → **立即烹調失敗**，食物燒焦
- 設計意圖：強制玩家只在提示出現時才操作，增加互動技巧門檻

#### 進階烹飪配方（食材追加系統）
- YAML 新欄位：`advanced: true`, `perfect-output-count: N`, `ingredient-additions`
- 進階配方在烹調過程中，會在指定進度百分比提示「加入指定材料」
- 玩家需手持正確材料右鍵烹飪台投入（窗口 4 秒）
- 成功加入 → 加速烹調 + 食材粒子特效
- 全部 QTE 通過 + 全部食材追加 → **完美烹調**，一次掉落 `perfect-output-count` 份成品

#### 5 道新高階料理
| 料理 | 主要效果 | 效果時長 | 食用冷卻 | 完美產出 |
|------|---------|---------|---------|---------|
| 龍炎牛排 | 力量 II + 火焰抗性 + 抗性 II | 10s | 5 分鐘 | 4 份 |
| 虛空拉麵 | 速度 III + 夜視 + 緩降 | 8-15s | 4 分鐘 | 3 份 |
| 星輝燉飯 | 吸收 IV + 再生 III + 抗性 II | 8-10s | 6 分鐘 | 4 份 |
| 鳳凰湯 | 火焰抗性 + 再生 III + 力量 | 6-15s | 4.5 分鐘 | 3 份 |
| 奧術茶 | 急迫 III + 速度 II + 夜視 | 10-20s | 3.5 分鐘 | 3 份 |

設計原則：效果極強但時長極短，搭配長食用冷卻，作為 Boss 戰 / 危急狀況的爆發型補給。

### 研究台改動
- 舊行為：tick 只播粒子與音效，無實際功能（研究 UI 透過科技書也可開啟）
- 新行為：**被動科技經驗場** — 每秒對 8 格範圍內的玩家獎勵 1 點科技經驗
- 右鍵仍開啟研究台 UI
- 設計意圖：給研究台明確被動價值，鼓勵玩家在主基地放置研究台獲得持續經驗收益
